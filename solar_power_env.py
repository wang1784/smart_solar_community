...
class solar_power_env():
    def __init__(self):
        ...
        self._battery_bin = list(np.linspace(0, self._battery_cap, 4)) #make 3 bins for battery level:0-400, 401-800, 801-1200
        self._battery_power = 0 #records the exact power that the battery has
        self._battery_state = 0 #records the battery level as part of the state: 0, 1, 2 corresponding to each battery bin
        #power data
    ...
    def extract_from_table(self): #finds p_solar, p_comed
    ...
    def battery_level(self, change): #finds battery level with given change in power stored in battery
        ...
        for each_bin_min in self._battery_bin:
            if self._battery_power > each_bin_min:
                self._battery_state = self._battery_bin.index(each_bin_min)
            else: break

    def get_state(self, change): #combine three pieces of state information
        p_solar, p_comed = self.extract_from_table()
        self._state = [p_solar, p_comed, self.battery_level(change)]
    ...
    def get_reward(self, action):
        ...
        if action: #if action==1, charging
            reward = min(p_solar, self._battery_cap - self._battery_power)
        else: #if action==-1, discharging
            reward = min(p_load, self._battery_power)
        return reward
